...
E(gra)$weight <- 1 - (E(gra)$weight/max(E(gra)$weight)) #normalized distances
df <- get.data.frame(gra)
head(df)
### HACER UNA COPIA DEL GRAFICO CON NODOS FILTRADOS
graph_copy <- delete.edges(gra, which(E(gra)$weight < 0.97))
V(graph_copy)$label <- ""
plot(graph_copy, vertex.size = 3)
