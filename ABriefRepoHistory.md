...
Fortunately it is not, and that same day I started giving wings to my little demons. For those in process, I created container files with specific locations and the challenge was to get them to work from there (bin), read from (data) and send everything to (results). My ssDNA_viral_pangenomics repository was born and it was great to have reliable and automatic change control for the first time.
...
The date is someday in January 2021, after editing my scripts numbered 1-6, I just uploaded my last commit for pre-managing data for my pangenomic analysis. The 0.1_Master_Driver.sh script is the first code file that I would feel good about sharing. My work can be summarized as:

1. Download genomes
2. Filter database
3. Cluster by distances

A few months ago I presented the results for a viral family up to step 3. Today, after downloading the raw data, it takes no more than a couple of minutes to generate the 3 steps for a family, with textual and graphic reports. This is my work operating on all the genomes available for the Monodnaviria taxon (all single-stranded DNA viruses):
...
# Analyzes and discussion 
## So what's in the repo right now?
I'll show some results to guide you. However, it has to be restricted to data manipulation since merely biological interpretation will arise later after computing phylogenies and remote homologs search. 

Consider the following steps from 0.1_Master_Driver.sh:

### From raw to filtered database

The first three steps of my workflow fulfill the task of, separating a raw file into individual per species files, then concatenate segments files belonging to the same genome and then filtering outliers based on the count of proteins. Everything start from a single family.gb file.
...
```
The only remaining script bellonging to this section is the filtering by protein count. It's objective (also the main goal of pre data managing) is to prevent pangenomic core underestimation by the inclusion of outliers. The script that performs this steps need some parameters, so it does not always yield the same outcome. I had the self proclaimed great idea of including a table in stdout that summarizes the what if...? for the possible allowed variations, it will surely help to decide the cutoff percentage allowing to consider the number of remaining files on each scenario. This is the what if table for Monodnaviria:
...
```
### From filtered db to distance matrices

Paired distances are pure gold for evolutionary analyzes. There are lots of phylogeny inference methods that rely on paired distances. So... we need a multiple aligment right?. Guess what.. NOOOOO, they are great but you can't always use them, with Monodnaviria that'd never end. Remember, this repo is fast and furious, we'll try to keep a huge database while possible, and I'll try to make it possible. Thats how i got my AF-Abe nickname. 

I implemented a very interesting AF-method that relies in Fast Fourier Transforms, to estimate a 28 dimensional vector that corresponds 1 to 1 to a genome. That's an incredible property, because different to multiple aligments (that correspond many vs many), incorporating a new genome means to add a line and column to the paired distance matrix, not a complete re estimation of paired distances. There's no clear reason to show a distance matrix but here's one anyway.
...
Finally I got my files, safe and sound in folders ready to serve as pangenomic input clusters. To demonstrate it:
